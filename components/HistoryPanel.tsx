...
const HistoryItem: React.FC<{ item: GeneratedContent; onUseImage: (url: string) => void; onDownload: (url: string, type: string) => void; }> = ({ item, onUseImage, onDownload }) => {
    ...
    // Text-based result (e.g., Outfit Stylist)
    if (item.text && !item.imageUrl && !item.videoUrl && item.originalImageUrl) {
        return (
            <div className="bg-[var(--bg-secondary)] p-3 rounded-lg border border-[var(--border-primary)] flex flex-col gap-3">
                <img src={item.originalImageUrl} className="rounded-md w-full object-contain bg-[var(--bg-primary)]" alt="Original for advice" />
                <div className="text-xs text-center text-[var(--text-secondary)] -mb-1">{t('history.outfitAdvice')}</div>
                <p className="text-xs text-[var(--text-secondary)] bg-[var(--bg-primary)] p-2 rounded-md max-h-24 overflow-y-auto whitespace-pre-wrap">{item.text}</p>
                <div className="grid grid-cols-1 gap-2 text-sm">
                     <ActionButton onClick={() => onUseImage(item.originalImageUrl!)} isPrimary>
